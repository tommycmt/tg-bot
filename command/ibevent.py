...
def scrape_ibevent(week):
    url = "http://game.capcom.com/world/steam/hk/schedule-master.html"
...
def format_ibevent_kb():
    custom_keyboard = [[telegram.InlineKeyboardButton(text="week 1", callback_data="ibevent1"),
                       telegram.InlineKeyboardButton(text="week 2", callback_data="ibevent2"),
                       telegram.InlineKeyboardButton(text="week 3", callback_data="ibevent3")]]
    reply_markup = telegram.InlineKeyboardMarkup(custom_keyboard)
    return reply_markup

def format_ibevent_message(time_period, title_tags, content_tags):
...
# handle the command
def handle_ibevent(update):
    time_period, title_tags, content_tags = scrape_ibevent("1")
    
    text = format_ibevent_message(time_period, title_tags, content_tags)
    text += "資料來源：Monster Hunter：World：CAPCOM"
    
    reply_markup = format_ibevent_kb()
    update.message.reply_text(text, reply_markup=reply_markup, parse_mode = "HTML")
    
    
# handle the callback query
def callback_ibevent(update):
    text = ""
    ...
    # Change pages
    week = data.replace("ibevent","")
    
    time_period, title_tags, content_tags = scrape_ibevent(week)
    text = format_ibevent_message(time_period, title_tags, content_tags)
    reply_markup = format_ibevent_kb()
